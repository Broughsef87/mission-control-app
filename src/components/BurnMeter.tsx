...
export default function BurnMeter({ logs }: { logs: TokenLog[] }) {
  ...
  return (
    <div className="bg-forge-panel/40 border border-white/5 rounded-sm p-6 backdrop-blur-sm relative overflow-hidden group hover:border-forge-red/30 transition-all">
      <div className="absolute top-0 right-0 p-3 font-mono text-[9px] text-ab-muted tracking-widest uppercase">The Burn // token economy</div>
      ...
      <div className="mb-6">
        <div className="flex justify-between items-end mb-2">
          <span className="text-[10px] font-mono text-ab-muted uppercase tracking-widest">Daily Spend</span>
          <span className="text-xl font-black text-ab-body">${totalCost.toFixed(2)}</span>
        </div>
        <div className="h-1.5 w-full bg-ab-border rounded-full overflow-hidden">
          <div 
        ...
        <div className="flex justify-between mt-1">
          <span className="text-[8px] font-mono text-ab-muted uppercase">0.00</span>
          <span className="text-[8px] font-mono text-ab-muted uppercase">Budget: ${dailyBudget.toFixed(2)}</span>
        </div>
      </div>

      <div className="space-y-2">
        <div className="flex justify-between text-[10px] border-b border-white/5 pb-2 mb-2">
          <span className="text-ab-muted font-mono">Agent</span>
          <span className="text-ab-muted font-mono">Tokens</span>
        </div>
        {logs.slice(0, 5).map(log => (
          <div key={log.id} className="flex justify-between text-[10px] items-center">
            <span className="font-bold text-ab-text/80">{log.agent}</span>
            <span className="font-mono text-ab-body">{log.tokens.toLocaleString()}</span>
          </div>
        ))}
      </div>
    </div>
