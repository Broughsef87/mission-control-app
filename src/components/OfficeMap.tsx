...
const AGENTS = [
  { id: 'devroux', name: 'Devroux', role: 'Ops / Commander', color: 'bg-ab-blue', pos: { x: 4, y: 3 }, avatar: '🤖' },
  { id: 'isaac', name: 'Isaac', role: 'Researcher', color: 'bg-ab-green', pos: { x: 1, y: 1 }, avatar: '🔍' },
  { id: 'charles', name: 'Charles', role: 'Producer', color: 'bg-purple-500', pos: { x: 1, y: 5 }, avatar: '🎬' },
  { id: 'max', name: 'Max', role: 'Engineer', color: 'bg-ab-gold', pos: { x: 7, y: 1 }, avatar: '💻' },
  { id: 'gabriel', name: 'Gabriel', role: 'Watchdog', color: 'bg-ab-red', pos: { x: 8, y: 6 }, avatar: '🛡️' },
...
export default function OfficeMap() {
  ...
  return (
    <div className="w-full h-full bg-[#111] p-8 flex items-center justify-center relative overflow-hidden rounded-xl border border-[#333]">
      {/* Grid Floor Pattern */}
      ...
      <div className="relative w-[800px] h-[500px] bg-ab-surface-2 rounded-lg border-4 border-[#444] shadow-[0_0_50px_rgba(0,0,0,0.5)] grid grid-cols-9 grid-rows-7 gap-2 p-4">
        
        {/* Desks (Static Props) */}
        {AGENTS.map(agent => (
           <div key={`desk-${agent.id}`} 
                className="absolute w-16 h-10 bg-ab-surface-2 border-t-4 border-[#555] rounded-sm shadow-md flex items-center justify-center"
                style={{ 
        ...
        <div className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-48 h-24 bg-ab-surface-2 rounded-full border-4 border-[#444] flex items-center justify-center">
            ...
            >
              ...
              {isActive && (
                <motion.div 
                  ...
                  className="absolute -top-8 bg-[#222] text-ab-body text-[9px] px-2 py-1 rounded border border-[#444] whitespace-nowrap z-20"
                >
                  <span className="w-1.5 h-1.5 bg-ab-green rounded-full inline-block mr-1 animate-pulse"></span>
                  Working...
                </motion.div>
              )}
              ...
              {/* Name Tag */}
              <span className={`mt-1 text-[9px] font-bold px-1 rounded bg-black/50 ${isActive ? 'text-ab-body' : 'text-ab-muted'}`}>
                {agent.name}
              </span>
            </motion.div>
          );
      ...
      <div className="absolute bottom-6 left-6 text-ab-muted text-xs font-mono">
        The Foundry // OFFICE VIEW
      </div>
    </div>
  );
}
