...
export default function TaskList({ refreshTrigger }: { refreshTrigger: number }) {
  ...
  return (
    <div className="bg-ab-surface-2 border border-ab-border rounded-lg p-6 max-h-[400px] overflow-auto hover:border-gray-700 transition-colors shadow-lg">
      <h3 className="text-xs font-semibold text-gray-500 mb-4 uppercase tracking-widest flex items-center gap-2">
        <ListTodo size={14} className="text-ab-gold" />
        Pending Objectives
      </h3>

      <div className="space-y-3">
        {tasks.map((task) => (
          <div key={task.id} className="flex items-start justify-between p-3 bg-ab-surface-2 rounded border border-ab-border hover:border-gray-600 transition-colors group">
             <div className="flex items-start gap-3 flex-1 pr-4">
               <button className="text-gray-600 hover:text-ab-green mt-0.5 transition-colors">
             ...
             <button 
                ...
                className="opacity-0 group-hover:opacity-100 transition-opacity p-1.5 hover:bg-ab-border-bright rounded-md text-gray-400 hover:text-ab-blue shrink-0"
        ...
        {tasks.length === 0 && (
           <div className="flex flex-col items-center justify-center py-10 text-gray-700 border border-dashed border-ab-border rounded">
